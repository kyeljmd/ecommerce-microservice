package com.kyeljmd.ecormmerce.shopeepee.web.rest;

import com.kyeljmd.ecormmerce.shopeepee.security.SecurityUtils;
import com.kyeljmd.ecormmerce.shopeepee.service.CartService;
import com.kyeljmd.ecormmerce.shopeepee.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.kyeljmd.ecormmerce.shopeepee.domain.Cart}.
 */
@RestController
@RequestMapping("/api")
public class CartResource {
    ...
    private static final String ENTITY_NAME = "shopeepeeCart";
    ...
    public ResponseEntity<CartDTO> createCart(@RequestBody CartDTO cartDTO) throws URISyntaxException {
        ...
        log.debug("REST request to save Cart : {} For User {}", cartDTO, owner);
        if (cartDTO.getId() != null) {
            throw new BadRequestAlertException("A new cart cannot already have an ID", ENTITY_NAME, "idexists");
        }
        CartDTO result = cartService.save(cartDTO, owner);
    ...
    public ResponseEntity<List<CartDTO>> getAllCarts(Pageable pageable) {
        ...
        String owner = SecurityUtils.getCurrentUserLogin().get();
        Page<CartDTO> page = cartService.findByOwnerId(owner, pageable);
